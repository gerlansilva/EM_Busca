#!/usr/bin/env python3
"""
BuscaEM — coletor OAI-PMH v2
Correções:
- preserva variantes linguísticas usando xml:lang quando a fonte fornece;
- escolhe título/resumo/palavras-chave no idioma original do artigo;
- separa variantes em inglês em titleEn/abstractEn/keywordsEn;
- remove duplicações de autores e palavras-chave;
- constrói URL do artigo OJS quando o OAI não a fornece;
- usa ListRecords + resumptionToken e checkpoint.
"""
...
USER_AGENT = "BuscaEM/2.0 metadata-harvester (https://github.com/gerlansilva/EM_Busca)"
TIMEOUT = 45
SLEEP = 0.35
...
def values(parent, local):
    ...
    for el in parent.iter():
        if el.tag.split("}")[-1]==local and el.text:
...
def choose_keywords(vals, target_lang):
    ...
    # If OAI omitted xml:lang, use lightweight phrase-level language detection.
    detected=[v["text"] for v in vals if detect_lang(v["text"])==target_lang]
    if detected:
        return unique(detected)
    # Neutral technical terms cannot always be classified. Keep only once.
    return unique(v["text"] for v in vals)
...
def parse_dc_record(record,journal):
    ...
    if not lang:
        # infer from longest abstract, then title
...
def harvest_one(journal,limit=None,reset=False):
    ...
    if reset and state_file.exists(): state_file.unlink()
    ...
    while True:
        ...
        raw_path=RAWDIR/jid/"last-page.xml"; raw_path.parent.mkdir(parents=True,exist_ok=True); raw_path.write_bytes(raw)
        rows,next_token=parse_page(raw,journal)
        if limit is not None:
            rows=rows[:max(0,limit-seen)]
        existing=merge_records(existing,rows); seen+=len(rows)

        save(out_file,{"journal":jid,"journalName":journal["name"],"source":"oai-pmh",
                       "sourceUrl":journal["oaiBase"],"updatedAt":now(),"articles":existing})
        save(state_file,{"journal":jid,"resumptionToken":next_token,"pages":pages,
                         "recordsSeenThisRun":seen,"updatedAt":now(),"complete":not bool(next_token)})
        ...
        token=next_token; time.sleep(SLEEP)
    return len(existing)
...
def main():
    ...
    total=0
    for j in rows:
        try: total+=harvest_one(j,args.limit,args.reset)
        except Exception as e: print(f"ERROR [{j['id']}]: {type(e).__name__}: {e}")
